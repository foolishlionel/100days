...
```
#### 1.2 端口和IP的关系
IP地址是电脑砸网络中要来标记地址的一组数字，就像门牌号码一样。比如192.168.1.1；端口是电脑系统对应一些服务所开启的通信通道，比如打开网页要用的是80端口，telnet用的是23端口等等。socket（套接字），就是IP：端口号的形式。
...
这个标识好就是端口，其实网卡都是被系统层封装了，端口和进程之间的关系也是系统封装好的。我们只需要用socket就行，给定一个端口号就行。其他的事都交给操作系统去做。

端口只需要简单理解为一个标志就好，比如电脑一和电脑二建立tcp链接，简历一条没问题啊，双方处理链接上传报文就好饿了，但是要简历多条链接，怎么区分这两条链接呢？那就用一种标志吧，带个数字，这就是端口号。
